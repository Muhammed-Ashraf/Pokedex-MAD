...
import ashraf.pokedex.mad.core.data.repository.userdata.FakeUserDataRepository
...
@Composable
fun PokedexSettings(settingsViewModel: SettingsViewModel = hiltViewModel()) {
    val uiState by settingsViewModel.uiState.collectAsStateWithLifecycle()
    // LocalWindowInfo.current provides information about the current Compose window state.
// The most common use is to check whether the window is currently focused or not.
//
// Example use cases:
// - Pause animations or timers when the app loses focus
// - Stop or reduce background work when user switches apps
// - Avoid unnecessary UI updates when window is not active
//
// Key property:
// - isWindowFocused: true when the app window is currently in focus (foreground/active)
    val windowInfo = LocalWindowInfo.current

// This sets the maximum width of the Box to (screen width - 80px).
// windowInfo.containerSize.width gives the available window width in pixels,
// so we subtract 80 to create horizontal spacing on both sides.
//
// ⚠️ Note:
// - containerSize.width is in pixels, not dp
// - converting to .dp directly can lead to unit mismatch issues
//
// Result:
// The Box will never exceed the screen width minus 80px, leaving side margins.
    Box(
        modifier = Modifier
        ...
        SettingDialog(settingsUiState = uiState, onChangeUiTheme = settingsViewModel::setUiTheme)
    }
}

@Composable
fun SettingDialog(settingsUiState: SettingsUiState, onChangeUiTheme: (uiTheme: UiTheme) -> Unit) {
